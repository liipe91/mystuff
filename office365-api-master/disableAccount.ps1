...
if ($MailNickName.Length -lt 2 )  {
    Write-Warning "Usuario invalido";
    Write-Warning "Usage .\disableAccount.ps1 <apenas_iniciais>";
    break }
...
if ($UObjID.Length -lt 30) {
    ...
    break
} elseif ($AccountEnabled -match "False") {
    Write-Host "Usuario $MailNickName ja desabilitado" -ForegroundColor Yellow ;
    break
}
...
Write-Host "Desabilitando conta usuario $UserPrincipalName" -ForegroundColor Green
Set-AzureADUser -ObjectId $UObjID -AccountEnabled $False
